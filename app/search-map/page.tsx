  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const l of withoutCoords) {
        ...
        const q = (typeof anyAddr === 'string' ? anyAddr : (anyAddr?.en || anyAddr?.bg || anyAddr?.ru)) || (l as any).title?.en || (l as any).slug || (l as any).location?.name?.en;
        if (!q) continue;
        try {
          ...
          if (feat?.center) {
            updates[l.id] = [feat.center[0], feat.center[1]];
          }
        } catch {}
      }
